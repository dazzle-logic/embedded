...
// switch mode
#define MODE_RESET 0 // SW[3]
#define MODE_1 1     // SW[0] - Timer (All ON/OFF)
#define MODE_2 2     // SW[1] - Timer (Sequential ON)
#define MODE_3 3     // SW[2] - Immediate Toggle
#define HIGH 1
#define LOW 0
...
irqreturn_t irq_handler(int irq, void *dev_id)
{
    ...
    if (sw_index == 3) {
        printk(KERN_INFO "sw4 interrupt occurred! (RESET)\n");
        start_mode_reset();
        ...
        switch (sw_index) {
        case 0:
            printk(KERN_INFO "sw1 interrupt occurred! (MODE 1)\n");
            current_mode = MODE_1;
            start_mode_1();
            break;

        case 1:
            printk(KERN_INFO "sw2 interrupt occurred! (MODE 2)\n");
            current_mode = MODE_2;
            start_mode_2();
            break;

        case 2:
            printk(KERN_INFO "sw3 interrupt occurred! (MODE 3)\n");
            current_mode = MODE_3;
            break;
...
static int __init switch_module_exam1_init(void)
{
    ...
    for (i = 0; i < 4; i++) {
        res = request_irq(sw_irq[i], (irq_handler_t)irq_handler,
                          IRQF_TRIGGER_RISING, "IRQ", (void *)irq_handler);
        if (res < 0)
